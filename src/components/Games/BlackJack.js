...
export default class Blackjack extends React.Component {
    ...
    constructor() {
      ...
      componentDidMount() {
        firebase.firestore().collection("BlackjackTables").onSnapshot((snapshot) => {
            snapshot.forEach((doc) => {
                this.setState(prevState => ({
                    tables: [...prevState.tables, {id: doc.id, name: doc.data().tableName, players: doc.data().numPlayers}],
      ...
      render() {
        ...
        return (
            <div>
                {this.state.tableId ? 
                <Table tableId={this.state.tableId} username={this.props.username} />
                :
                this.state.tables.map((table) => {
                    return <Button key={table.id} onClick={() => this.setState({tableId: table.id})}> 
                    {table.name}
                    <br />
